...
This is the quickest way to start using CKEditor in your project. Assuming Vue is installed (official guide for [version 2.x](https://vuejs.org/v2/guide/installation.html) and for [version 3.x](https://v3.vuejs.org/guide/installation.html)), include the `<script>` tags for the WYSIWYG editor component and the build:
...
```js
// Only for Vue 3.x
Vue.createApp( {} ).use( CKEditor( Vue ) );
```
...
createApp( {} ).use( CKEditor( { version, h } ) );
```

Please note that the CKEditor is a function and it requires information about the Vue version that is used in your application. For Vue 3.x the CKEditor function also requires the `h` function. The following examples show different possibilities of creating a plugin and a component, depending on your Vue version:
...
// but the 'h' parameter is also mandatory and it must come from the Vue package.
createApp( {} ).use( CKEditor( { version: 3, h } ) );
```
...
A standard directive (official description [for Vue 2.x](https://vuejs.org/v2/api/#v-model) and [for Vue 3.x](https://v3.vuejs.org/guide/component-basics.html#using-v-model-on-components)) for form inputs in Vue. Unlike [`model-value`](#model-value), it creates a two–way data binding, which:
