...
<info-box>
	The component is compatible with Vue.js 2.x and Vue.js 3.x versions. This documentation provides examples for both versions of Vue.js.
</info-box>
...
This is the quickest way to start using CKEditor in your project. Assuming Vue is installed (check out the official guides for [Vue 2.x](https://vuejs.org/v2/guide/installation.html) and for [Vue 3.x](https://v3.vuejs.org/guide/installation.html) to learn more), include the `<script>` tags for the WYSIWYG editor component and the build:
...
If you are using Vue 2.x, enable the component globally in your application by using the [`Vue.use()`](https://vuejs.org/v2/api/#Vue-use) method:

```js
// Vue 2.x only.
Vue.use( CKEditor( Vue ) );
```

If you have Vue 3.x, use the [application instance](https://v3.vuejs.org/guide/instance.html#creating-an-application-instance) to install the component globally:

```js
// Vue 3.x only.
Vue.createApp( { /* options */ } ).use( CKEditor( Vue ) );
```
...
To create an editor instance, you must first import the editor build and the component modules into the root file of your application (e.g. `main.js` when generated by Vue CLI). Then, enable the component depending on the Vue.js version you have - either using the [`Vue.use()`](https://vuejs.org/v2/api/#Vue-use) method (in Vue 2.x):
...
or using the [application instance](https://v3.vuejs.org/guide/instance.html#creating-an-application-instance) in Vue 3.x:
...
createApp( { /* options */ } ).use( CKEditor( { version, h } ) );
```

Please note that `CKEditor` is a function and it requires information about the Vue version that is used in your application. For Vue 3.x, the CKEditor function also requires the [`h()` function](https://v3.vuejs.org/guide/render-function.html). The following examples show different possibilities of creating a plugin and a component, depending on your Vue version:
...
// CKEditor function defaults to Vue 2.x,
Vue.use( CKEditor() );
// ...or you can pass the whole Vue constructor,
Vue.use( CKEditor( Vue ) );
// ...or you can pass only required version,
Vue.use( CKEditor( { version: Vue.version } ) );
// ...or you can specify the target version manually.
Vue.use( CKEditor( { version: 2 } ) );
```
...
// For Vue 3.x, CKEditor function requires both parameters: target version and the 'h' function.
createApp( { /* options */ } ).use( CKEditor( { version, h } ) );
// The version can be provided manually, just like for Vue 2.x,
// but the 'h' parameter is mandatory and must come from the Vue package.
createApp( { /* options */ } ).use( CKEditor( { version: 3, h } ) );
```
...
<script>
	const component = CKEditor( Vue ).component;
...
A standard directive (check out [Vue 2.x](https://vuejs.org/v2/api/#v-model) and [Vue 3.x](https://v3.vuejs.org/guide/component-basics.html#using-v-model-on-components) API documentation to learn more) for form inputs in Vue. Unlike [`model-value`](#model-value), it creates a two–way data binding, which:
