...
The {@link module:comments/annotations/sidebar~Sidebar `Sidebar`} class stopped operating on the global collection of annotations. From now, the Annotation UI that is responsible for displaying the sidebar should provide the sidebar with its collection of annotations and inform about possible actions with the new {@link module:comments/annotations/sidebar~Sidebar#function-rearrange `rearrange()`} and {@link module:comments/annotations/sidebar~Sidebar#function-refresh `refresh()`} methods.
