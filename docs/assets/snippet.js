...
/**
 * Find and return toolbar item using one of several ways. List of options sorted in priority of execution:
 * 		- callback (comparison function)
 * 		- item label
 * 		- item position
 *
 * If label property would not be found inside an item, there will be comparison to item.buttonView.label.
 * If item with exact label would not be found, function will try to find item that begin with provided label name.
 *
 * @param {module:ui/toolbar/toolbarview~ToolbarView} toolbarView
 * @param {Object} [options]
 * @param {String} [options.label] Label name of an item to find (case sensitive).
 * @param {Number} [options.position] Position on a toolbar of an item to find.
 * @param {Function} [callback] Comparison function
 * @returns {HTMLElement|undefined} HTML element or undefined
 */
window.findToolbarItem = function( toolbarView, { label, position }, callback ) {
	const items = toolbarView.items;
	let item;

	if ( callback ) {
		item = items.find( callback );
	} else if ( label ) {
		item = items.find( getFindViewItemByLabelCallback( label ) ) ||
			items.find( getFindViewItemByLabelCallback( label, true ) );
...
/**
 * Returns callback used by {@link module:ui/viewcollection~ViewCollection~find} method to find view item.
 *
 * @param {String} [label]
 * @param {Boolean} [startsWith]
 * @returns {function(module:ui/view~View): boolean}
 */
function getFindViewItemByLabelCallback( label, startsWith = false ) {
	return item => {
		const itemLabel = !item.label && item.buttonView && item.buttonView.label ?
			item.buttonView.label :
			item.label;
		return startsWith ?
			itemLabel.startsWith( label ) :
			itemLabel === label;
	};
}

// eslint-disable-next-line no-undef
window.addItemIndicator = ( typeof addItemIndicator !== 'undefined' ) ? addItemIndicator : function() {};
