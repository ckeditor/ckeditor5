...
will create a "Basic styles" drop-down with a "text" icon containing "bold" and "italic" buttons
...
You can use one of the icons listed below for your drop-down:
...
toolbar: [
	...
	},
	{
		...
		// If you want your icon to change the color dynamically (e.g. when the drop-down opens), avoid fill="..."
...
By default, the tooltip of the button shares its text with the label. You can customize it to better describe your drop-down using the `tooltip` property ({@link module:ui/button/buttonview~ButtonView#tooltip learn more}):
