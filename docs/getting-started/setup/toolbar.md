...
The toolbar offers a flexible arrangement, achieved through configuration. Please note, that using [CKEditor&nbsp;5 Builder](https://builder.ckeditor.com/?redirect=docs) makes this task significantly easier.
...
The block toolbar complements the [balloon editor type](https://builder.ckeditor.com/?redirect=docs) where it falls short, for example when you must insert some content (like an image), but the selection is collapsed, so you cannot access the toolbar. It can, however, be added to any type of editor and configure accordingly (see below).
