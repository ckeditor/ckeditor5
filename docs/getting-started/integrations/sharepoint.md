...
The `!important` rule overrides any styles generated by the editor, so setting them through table properties brings no effect. To fix it, you need to make sure CKEditor&nbsp;5's internal styles take precedence before SharePoit's global styles. This can be achieved by introducing the following workaround, in the form of a custom plugin:
