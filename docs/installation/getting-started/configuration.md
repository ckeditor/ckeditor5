...
As CKEditor builds do not include all possible features, the only way to add more features to them is to {@link  link-removed create a custom build}.
...
Some of the options may require loading plugins which are not available in the build you use. Read more about {@link  link-removed customizing builds}.
