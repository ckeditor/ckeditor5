...
A collaboration session will be removed 24h after the last user disconnects, but you can manually [flush](https://help.cke-cs.com/api/v4/docs#tag/Collaboration/paths/~1collaborations~1{document_id}/delete) the existing collaboration sessions using {@link @cs guides/apis/overview Cloud Services REST API}. After collaboration session removal, users will be able to connect to the documents using a new editor.
