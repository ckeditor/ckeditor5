...
<info-box>
	Please keep in mind, that this new mechanism does not affect the {@link installation/legacy-getting-started/getting-and-setting-data data saved by the editor} (e.g. the output of `editor.getData()`). The filtering only applies during the editing when the user interacts with the editor.
</info-box>
