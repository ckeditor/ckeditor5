...
* nodes can be disallowed in certain places. For example, if a custom `specialParagraph` inherits all properties from `paragraph` but needs to disallow `imageInline`.
* attributes can be disallowed in a certain node. For example, a custom `specialPurposeHeading` inherits attributes from `heading2` but does not allow `alignment` attribute.
