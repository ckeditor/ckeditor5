...
* `--debug` (alias `-d`) &ndash; Allows specifying custom debug flags. For example, the `--debug engine` option uncomments the `// @if CK_DEBUG_ENGINE //` lines in the code. The `--debug false` turns off the debugging mechanism and leaves the lines starting with `// @if CK_DEBUG //` as they are.
