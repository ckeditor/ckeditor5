...
This guide will describe what is available inside `JavaScript` package generated by the {@link framework/guides/package-generator/using-package-generator Package Generator}.
...
<info-box hint>
    You can run `npm run dll:build -- --watch` and `npm run dll:serve` in two separate command terminals. That way, after saving your changes and reloading the page, the content will update.
</info-box>
