...
function performTest() {
	...
	const initialData = allDataSets[ dataSetName as keyof typeof allDataSets ];
...
function setStatus() {
	...
	statusEl!.textContent = `Dataset "${ dataSetName }" (${ dataSetIndex + 1 } / ${ dataSetsNames.length }), ` +
...
function saveResult( time: number ) {
...
function nextTry( tryNumber: number ) {
