...
class AbbreviationUI extends Plugin {
	...
	public _createToolbarAbbreviationButton() {
		const editor = this.editor;
	...
	public _showUI() {
		...
		const commandValue = this.editor.commands.get( 'addAbbreviation' )!.value as any;
...
class AbbreviationCommand extends Command {
	...
	public override execute( { abbr, title }: { abbr: string; title: string } ) {
		const model = this.editor.model;
		const selection = model.document.selection;
