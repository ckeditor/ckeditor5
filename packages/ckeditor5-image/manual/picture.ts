...
ClassicEditor
	...
	.then( editor => {
		...
		editor.plugins.get( 'ImageUploadEditing' ).on( 'uploadComplete', ( evt, { data } ) => {
			data.default = '/images/logo-wide.png';
			data.sources = [
				{ srcset: '/images/logo-square.png', type: 'image/png', media: '(max-width: 800px)' },
				{ srcset: '/images/logo-wide.png', type: 'image/png', media: '(min-width: 800px)' }
			];
		}, { priority: 'high' } );
	} )
	.catch( err => {
