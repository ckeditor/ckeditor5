...
declare global {
	interface Window {
		editorFormatting: any;
		editorSemantic: any;
		editorWithDropdown: any;
	}
}
...
async function startEditors() {
	window.editorSemantic = await ClassicEditor.create( {
		attachTo: document.querySelector( '#editor-semantic' ) as HTMLElement,
	...
	window.editorFormatting = await ClassicEditor.create( {
		attachTo: document.querySelector( '#editor-formatting' ) as HTMLElement,
		cloudServices: CS_CONFIG,
		plugins: PLUGINS_CONFIG,
		toolbar: TOOLBAR_CONFIG,
		image: {
	...
	window.editorWithDropdown = await ClassicEditor.create( {
		attachTo: document.querySelector( '#editor-with-dropdown' ) as HTMLElement,
		cloudServices: CS_CONFIG,
		plugins: PLUGINS_CONFIG,
		toolbar: TOOLBAR_CONFIG,
		image: {
...
function bindPreview( editor: any, previewId: string ) {
