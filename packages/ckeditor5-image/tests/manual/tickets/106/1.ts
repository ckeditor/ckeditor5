...
ClassicEditor.create( {
	...config,
	attachTo: document.querySelector( '#editor-insert' ) as HTMLElement
} )
	.then( editor => {
...
ClassicEditor.create( {
	...config,
	attachTo: document.querySelector( '#editor-delete' ) as HTMLElement
} )
	.then( editor => {
...
function startExternalInsert( editor: any ) {
	const model = editor.model;

	function type( path: Array<number>, text: string ) {
		return new Promise<void>( resolve => {
	...
	function insertNewLine( path: Array<number> ) {
...
function startExternalDelete( editor: any ) {
	const model = editor.model;
