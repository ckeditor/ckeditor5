...
function getConfig( resizeUnitInPx = false ) {
	...
	if ( resizeUnitInPx ) {
		( config.image as any ).resizeUnit = 'px';
	}
...
async function initEditors() {
	await Promise.all( editors.map( async editorObj => {
		...
		const editor = await ClassicEditor.create( {
			...editorObj.config,
			attachTo: domElement as HTMLElement,
			root: { initialData: editorObj.data }
		} );

		( window as any )[ editorObj.id ] = editor;
	...
	CKEditorInspector.attach( Object.fromEntries( editors.map( editorObj => [ editorObj.id, ( window as any )[ editorObj.id ] ] ) ) );
}
...
function insertEditorStructure( editorObj: any ) {
...
function logInitialData( domElement: any, editorObj: any ) {
...
function updateLogsAndData( domElement: any, editor: any ) {
