...
1. First, an image (or images) need to get into the rich-text editor content. There are many ways to do that, for instance:
	...
	* selecting an image through a file system dialog.
	...
	The images are intercepted by the {@link module:image/imageupload~ImageUpload image upload} plugin (which is enabled in all official {@link installation/predefined-builds editor builds}).
...
* **The {@link module:image/imageupload~ImageUpload image upload} plugin must be enabled** in the editor. It is enabled by default in all official {@link installation/predefined-builds builds}, but if you are {@link  link-removed customizing} CKEditor 5, do not forget to include it.
...
Having implemented the adapter, you must figure out how to enable it in the WYSIWYG editor. The good news is that it is pretty easy, and you do not need to {@link  link-removed rebuild the editor} to do that!
