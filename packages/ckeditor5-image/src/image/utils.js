...
export default class ImageUtils extends Plugin {
	...
	toImageWidget( viewElement, writer, label ) {
		...
		function labelCreator() {
			const imgElement = that.getViewImageFromWidget( viewElement );
	 ...
	 *
	 * Also, see {@link module:image/image/utils~ImageUtils#isImageWidget}.
	 *
	...
	insertImage( attributes = {}, selectable = null, imageType = null ) {
		...
		imageType = this._determineImageTypeForInsertion( selectable || selection, imageType );
	...
	isImageAllowed() {
		...
		return this._isImageAllowedInParent( selection, model.schema, this.editor ) && this._isNotInsideImage( selection );
	}
	 ...
	 */
	createImageViewElement( writer, imageType ) {
	 ...
	 */
	getImageTypeMatcher( matchImageType ) {
		const editor = this.editor;
		...
		return element => {
			...
			if ( !this.isInlineImageView( element ) || !element.hasAttribute( 'src' ) ) {
				return null;
			}

			// The <img> can be standalone, wrapped in <figure>...</figure> (ImageBlock plugin) or
			// wrapped in <figure><a>...</a></figure> (LinkImage plugin).
			const imageType = element.findAncestor( this.isBlockImageView ) ? 'image' : 'imageInline';
	 ...
	 */
	determineImageTypeForInsertionAtSelection( selection ) {
	 ...
	 * @returns {Boolean}
	 */
	_isImageAllowedInParent( selection, schema, editor ) {
		const imageType = this._determineImageTypeForInsertion( selection );

		if ( imageType == 'image' ) {
			const parent = this._getInsertImageParent( selection, editor.model );
	 ...
	 */
	_getInsertImageParent( selection, model ) {
	 ...
	 */
	_determineImageTypeForInsertion( selectable, imageType ) {
		const editor = this.editor;
		...
		if ( selectable.is( 'selection' ) ) {
			return this.determineImageTypeForInsertionAtSelection( selectable );
		}
