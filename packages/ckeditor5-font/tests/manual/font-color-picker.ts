...
ClassicEditor
	.create( {
		attachTo: document.querySelector( '#editor-with-default-picker' ) as HTMLElement,
...
ClassicEditor
	.create( {
		attachTo: document.querySelector( '#editor-with-custom-picker' ) as HTMLElement,
		image: { toolbar: [ 'toggleImageCaption', 'imageTextAlternative' ] },
		plugins: [
			ArticlePluginSet,
			FontColor,
...
ClassicEditor
	.create( {
		attachTo: document.querySelector( '#editor-without-picker' ) as HTMLElement,
		image: { toolbar: [ 'toggleImageCaption', 'imageTextAlternative' ] },
		plugins: [
			ArticlePluginSet,
			FontColor,
...
function updateText( styleName: any ) {
	return ( evt: any ) => {
		const el = document.querySelector( '#color-box > p > span' );
		if ( el ) {
			( el as HTMLElement ).style[ styleName ] = evt.target.value;
		}
	};
}
