...
export default class InsertTextCommand extends Command {
	...
	/**
	 * Executes the insert text command. It replaces the content within the given selection with the given text.
	 *
	 * Replacing is a two step process:
	 *
	 * 1. first the content within the selection is removed,
	 * 2. then the new text is inserted at the beginning of the selection (it contains a single collapsed range after the first step).
	 *
	 * @fires execute
	...
	execute( options = {} ) {
		...
		const textInsertionsLength = text.length;
		const selection = options.selection || modelDocument.selection;
		...
		model.enqueueChange( this._buffer.batch, writer => {
			...
			if ( text ) {
				const modelText = writer.createText( text, modelDocument.selection.getAttributes() );

				model.insertContent( modelText, selection );
			}
			...
			this._buffer.input( textInsertionsLength );
		} );
	}
}
