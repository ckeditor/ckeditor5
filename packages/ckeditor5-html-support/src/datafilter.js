	...
	_registerInlineElement( definition ) {
		...
		editor.model.document.registerPostFixer( writer => {
			...
			for ( const change of changes ) {
				if ( change.type == 'attribute' && change.attributeNewValue === null && change.attributeKey != attributeKey ) {
					for ( const { item } of change.range ) {
						const htmlAttributes = item.getAttribute( attributeKey );

						// Remove ghs attribute if it was not generated by GHS and there is no more other attributes
						// that generated that element.
						if ( htmlAttributes && !htmlAttributes.name ) {
							// TODO there might be a set of attributes and we need too know which are generating which elements (span vs a)
							if ( Array.from( item.getAttributeKeys() ).length == 1 ) {
								writer.removeAttribute( attributeKey, change.range );
								changed = true;
							}
						}
					}
