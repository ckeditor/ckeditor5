...
The remove format feature is intended to help users tidy up chunks of content from unnecessary formatting. Each editor feature brings its own content types to the WYSIWYG editor. If you do not want the unnecessary formatting to be enabled in the first place, you may want to consider {@link getting-started/legacy-getting-started/configuration#removing-features reducing the number of features} enabled in the editor.
...
Enable the `RemoveFormatLinks` plugin in the {@link getting-started/legacy-getting-started/configuration#adding-features configuration} and run the editor:
