	...
	describe( 'unicode', () => {
		describe( 'isHighSurrogateHalf', () => {
			it( 'should return true if given character is a high surrogate half', () => {
				...
				expect( isHighSurrogateHalf( String.fromCharCode( 55394 ) ) ).toBe( true );
			} );

			it( 'should return false if given character is a low surrogate half', () => {
				// '𨭎'.charCodeAt( 0 ); // 55394 --> high surrogate half.
				// '𨭎'.charCodeAt( 1 ); // 57166 --> low surrogate half.
				expect( isHighSurrogateHalf( String.fromCharCode( 57166 ) ) ).toBe( false );
			} );

			it( 'should return false if given character is not a surrogate half', () => {
				expect( isHighSurrogateHalf( 'a' ) ).toBe( false );
				expect( isHighSurrogateHalf( '𨭎' ) ).toBe( false );
			} );

			it( 'should return false for falsy values', () => {
				expect( isHighSurrogateHalf( null ) ).toBe( false );
				expect( isHighSurrogateHalf( undefined ) ).toBe( false );
				expect( isHighSurrogateHalf( false ) ).toBe( false );
				expect( isHighSurrogateHalf( 0 ) ).toBe( false );
			} );
		} );
		...
		describe( 'isLowSurrogateHalf', () => {
			it( 'should return true if given character is a low surrogate half', () => {
				// '𨭎'.charCodeAt( 0 ); // 55394 --> high surrogate half.
				// '𨭎'.charCodeAt( 1 ); // 57166 --> low surrogate half.
				expect( isLowSurrogateHalf( String.fromCharCode( 57166 ) ) ).toBe( true );
			} );

			it( 'should return false if given character is a high surrogate half', () => {
				// '𨭎'.charCodeAt( 0 ); // 55394 --> high surrogate half.
				// '𨭎'.charCodeAt( 1 ); // 57166 --> low surrogate half.
				expect( isLowSurrogateHalf( String.fromCharCode( 55394 ) ) ).toBe( false );
			} );

			it( 'should return false if given character is not a surrogate half', () => {
				expect( isLowSurrogateHalf( 'a' ) ).toBe( false );
				expect( isLowSurrogateHalf( '𨭎' ) ).toBe( false );
			} );

			it( 'should return false for falsy values', () => {
				expect( isLowSurrogateHalf( null ) ).toBe( false );
				expect( isLowSurrogateHalf( undefined ) ).toBe( false );
				expect( isLowSurrogateHalf( false ) ).toBe( false );
				expect( isLowSurrogateHalf( 0 ) ).toBe( false );
			} );
		} );

		describe( 'isCombiningMark', () => {
			it( 'should return true if given character is a combining mark', () => {
				expect( isCombiningMark( '̣' ) ).toBe( true );
			} );

			it( 'should return false if given character is not a combining mark', () => {
				expect( isCombiningMark( 'a' ) ).toBe( false );
				expect( isCombiningMark( 'q̣' ) ).toBe( false );
			} );

			it( 'should return false for falsy values', () => {
				expect( isCombiningMark( null ) ).toBe( false );
				expect( isCombiningMark( undefined ) ).toBe( false );
				expect( isCombiningMark( false ) ).toBe( false );
				expect( isCombiningMark( 0 ) ).toBe( false );
			} );
		} );
		...
		describe( 'isInsideSurrogatePair', () => {
			...
			it( 'should return true if given offset in a string is inside a surrogate pair', () => {
				expect( isInsideSurrogatePair( testString, 2 ) ).toBe( true );
			} );

			it( 'should return false if given offset in a string is not inside a surrogate pair', () => {
				expect( isInsideSurrogatePair( testString, 1 ) ).toBe( false );
				expect( isInsideSurrogatePair( testString, 3 ) ).toBe( false );
			} );

			it( 'should return false if given offset in a string is at the string beginning or end', () => {
				expect( isInsideSurrogatePair( testString, 0 ) ).toBe( false );
				expect( isInsideSurrogatePair( testString, 4 ) ).toBe( false );
			} );

			it( 'should return false if given offset is between two surrogate pairs', () => {
				expect( isInsideSurrogatePair( '𨭎𨭎', 2 ) ).toBe( false );
			} );
		} );
		...
		describe( 'isInsideCombinedSymbol', () => {
			...
			it( 'should return true if given offset in a string is inside a symbol with combining mark', () => {
				expect( isInsideCombinedSymbol( testString, 2 ) ).toBe( true );
				expect( isInsideCombinedSymbol( testString, 3 ) ).toBe( true );
				expect( isInsideCombinedSymbol( testString, 4 ) ).toBe( true );
			} );

			it( 'should return false if given offset in a string is not inside a symbol with combining mark', () => {
				expect( isInsideCombinedSymbol( testString, 1 ) ).toBe( false );
				expect( isInsideCombinedSymbol( testString, 5 ) ).toBe( false );
			} );

			it( 'should return false if given offset in a string is at the string beginning or end', () => {
				expect( isInsideCombinedSymbol( testString, 0 ) ).toBe( false );
				expect( isInsideCombinedSymbol( testString, 6 ) ).toBe( false );
			} );
		} );
		...
		describe( 'isInsideEmojiSequence', () => {
			...
			for ( const [ sequence, name ] of testEmojiSequences ) {
				it( `should return true if given offset in a string is inside ${ name }`, () => {
					...
					expect( isInsideEmojiSequence( testString, 4 ) ).toBe( true );
				} );
			}
			...
			for ( const [ sequence, name ] of nonEmoiSequences ) {
				it( `should return false if given offset in a string is inside ${ name }`, () => {
					...
					expect( isInsideEmojiSequence( testString, 4 ) ).toBe( false );
				} );
			}
			...
			it( 'should return false if given offset in a string is before emoji sequence', () => {
				...
				expect( isInsideEmojiSequence( testString, 0 ) ).toBe( false );
				expect( isInsideEmojiSequence( testString, 1 ) ).toBe( false );
				expect( isInsideEmojiSequence( testString, 2 ) ).toBe( false );
			} );

			it( 'should return false if given offset in a string is at the first character of emoji sequence', () => {
				const testString = 'foo\u{1F1E7}\u{1F1EA}bar';

				expect( isInsideEmojiSequence( testString, 3 ) ).toBe( false );
			} );

			it( 'should return false if given offset in a string is after emoji sequence', () => {
				const testString = 'foo\u{1F1E7}\u{1F1EA}bar';

				expect( isInsideEmojiSequence( testString, 7 ) ).toBe( false );
				expect( isInsideEmojiSequence( testString, 8 ) ).toBe( false );
				expect( isInsideEmojiSequence( testString, 9 ) ).toBe( false );
			} );
			...
			it( 'should return true if given offset in a string is at any character in emoji sequence except the first one', () => {
				...
				expect( isInsideEmojiSequence( testString, 4 ) ).toBe( true );
				expect( isInsideEmojiSequence( testString, 5 ) ).toBe( true );
				expect( isInsideEmojiSequence( testString, 6 ) ).toBe( true );
			} );

			it( 'should return false if a string is empty', () => {
				expect( isInsideEmojiSequence( '', 0 ) ).toBe( false );
			} );
			...
			it( 'should properly detect multiple emojis in the same string', () => {
				...
				for ( let i = 0; i < testString.length - 1; i++ ) {
					...
					expect( isInsideEmojiSequence( testString, i ) ).toEqual( isInsideFirstEmoji || isInsideSecondEmoji );
				}
			} );
		} );
	} );
