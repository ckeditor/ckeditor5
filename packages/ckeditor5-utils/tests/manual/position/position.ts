...
const sources = document.querySelectorAll( '.source' ) as NodeListOf<HTMLElement>;
...
const positions = {
	...
	ne: ( targetRect: Rect, sourceRect: Rect ) => ( {
		top: targetRect.top,
		left: targetRect.right - sourceRect.width,
		name: 'ne'
	} ),
	sw: ( targetRect: Rect, sourceRect: Rect ) => ( {
		top: targetRect.bottom - sourceRect.height,
		left: targetRect.left,
		name: 'sw'
	} ),
	se: ( targetRect: Rect, sourceRect: Rect ) => ( {
		top: targetRect.bottom - sourceRect.height,
		left: targetRect.right - sourceRect.width,
		name: 'se'
	} )
...
setTimeout( () => {
	for ( const source of sources ) {
		const position = getOptimalPosition( {
			...
			target,
			positions: [
				positions[ source.className.split( '-' )[ 1 ] as keyof typeof positions ]
			],
