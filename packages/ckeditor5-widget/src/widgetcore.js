 ...
 */
export default class WidgetCore extends Plugin {
	/**
	 * @inheritDoc
	 */
	 ...
	 */
	static get requires() {
		return [ 'Widget', WidgetTypeAround ];
	}
		...
		this.editor.editing.downcastDispatcher.on( 'selection', ( evt, data, conversionApi ) => {
			...
			for ( const range of viewSelection.getRanges() ) {
				for ( const value of range ) {
					...
					if ( widget.isWidget( node ) && !isChild( node, lastMarked ) ) {
						viewWriter.addClass( widget.WIDGET_SELECTED_CLASS_NAME, node );
						...
						if ( node == selectedElement ) {
							viewWriter.setSelection( viewSelection.getRanges(), {
								fake: true,
								label: widget.getLabel( selectedElement )
							} );
						}
					}
				}
	...
	_onMousedown( eventInfo, domEventData ) {
		...
		if ( isInsideNestedEditable( element, widget ) ) {
		...
		if ( !widget.isWidget( element ) ) {
			element = element.findAncestor( widget.isWidget );
	...
	_clearPreviouslySelectedWidgets( writer ) {
		...
		for ( const widgetElement of this._previouslySelected ) {
			writer.removeClass( widget.WIDGET_SELECTED_CLASS_NAME, widgetElement );
		}
...
// @returns {Boolean}
function isInsideNestedEditable( element, widget ) {
	while ( element ) {
