...
describe( 'WidgetCore', () => {
	...
	beforeEach( () => {
		...
		return ClassicTestEditor
			...
			.then( newEditor => {
				...
				const widget = newEditor.plugins.get( 'Widget' );
	...
	it( 'should be loaded', () => {
		expect( editor.plugins.get( WidgetCore ) ).to.be.instanceOf( WidgetCore );
	} );
	...
	it( 'should require the WidgetTypeAround plugin', () => {
		expect( WidgetCore.requires ).to.include( WidgetTypeAround );
	} );

	it( 'should require the base Widget plugin as a soft-requirement', () => {
		expect( WidgetCore.requires ).to.include( 'Widget' );
	} );
	...
	describe( 'selection handle', () => {
		...
		beforeEach( () => {
			...
			return ClassicTestEditor
				.create( element, {
					plugins: [ Paragraph, Widget, WidgetCore, Typing ]
				} )
				.then( newEditor => {
					editor = newEditor;
					model = editor.model;
					...
					editor.conversion.for( 'downcast' )
						...
						.elementToElement( {
							model: 'widget',
							view: ( modelItem, { writer } ) => {
								const widgetElement = writer.createContainerElement( 'div' );

								return widget.toWidget( widgetElement, writer, { hasSelectionHandle: true } );
							}
						} )
						.elementToElement( {
							model: 'nested',
