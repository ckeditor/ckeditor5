...
describe( 'LinkImageEditing', () => {
	...
	describe( 'conversion in data pipeline (block images)', () => {
		...
		describe( 'view to model', () => {
			...
			describe( 'a > img', () => {
				it( 'should convert a link in an image figure', () => {
					editor.setData(
						'<figure><a href="http://ckeditor.com"><img src="/assets/sample.png" alt="alt text" /></a></figure>'
					);
				...
				it( 'should convert an image with a link and without alt attribute', () => {
					editor.setData( '<figure><a href="http://ckeditor.com"><img src="/assets/sample.png" /></a></figure>' );
				...
				it( 'should not convert in wrong context', () => {
					...
					editor.setData(
						'<div>' +
							'<figure><a href="http://ckeditor.com">' +
								'<img src="/assets/sample.png" alt="alt text" />' +
							'</a></figure>' +
						'</div>' );
				...
				it( 'should not convert if a link misses "href" attribute', () => {
					editor.setData(
						'<figure><a href=""><img src="/assets/sample.png" alt="alt text" /></a></figure>'
					);
			...
			describe( 'figure > a > img + figcaption', () => {
				it( 'should convert a link and the caption element', () => {
					return VirtualTestEditor
						.create( {
							plugins: [ Paragraph, ImageBlockEditing, ImageInlineEditing, LinkImageEditing, ImageCaptionEditing ]
						} )
						.then( editor => {
							editor.setData(
								'<figure class="image">' +
	...
	describe( 'conversion in data pipeline (inline images)', () => {
		describe( 'model to view)', () => {
			it( 'should attach a link indicator to the image element', () => {
				setModelData(
					model,
					'<paragraph><imageInline src="/assets/sample.png" alt="alt text" linkHref="foo"></imageInline></paragraph>'
				);
		...
		describe( 'model to data', () => {
			it( 'test should convert an image with a link', () => {
				setModelData( model,
					'<paragraph>' +
				...
				expect( editor.getData() ).to.equal(
					'<p><a href="http://ckeditor.com"><img alt="alt text" src="/assets/sample.png"></a></p>'
				);
			} );
			...
			it( 'should convert an image with a link and without alt attribute', () => {
				setModelData( model,
					'<paragraph>' +
						'<imageInline src="/assets/sample.png" linkHref="http://ckeditor.com"></imageInline>' +
					'</paragraph>'
				);

				expect( editor.getData() ).to.equal(
					'<p><a href="http://ckeditor.com"><img src="/assets/sample.png"></a></p>'
				);
			} );

			it( 'should convert srcset attribute to srcset and sizes attribute wrapped into a link', () => {
				setModelData( model,
					'<paragraph><imageInline src="/assets/sample.png" ' +
						'linkHref="http://ckeditor.com" ' +
						'srcset=\'{ "data": "small.png 148w, big.png 1024w" }\'>' +
		...
		describe( 'view to model', () => {
			describe( 'a > span > img', () => {
				it( 'should convert an image with a link and without alt attribute', () => {
					editor.setData(
						'<a href="http://ckeditor.com"><span class="image-inline"><img src="/assets/sample.png" /></span></a>'
					);

					expect( getModelData( model, { withoutSelection: true } ) )
						.to.equal(
							'<paragraph><imageInline linkHref="http://ckeditor.com" src="/assets/sample.png"></imageInline></paragraph>'
						);
				} );

				it( 'should not convert without src attribute', () => {
					editor.setData( '<a href="http://ckeditor.com"><span class="image-inline"><img alt="alt text" /></span></a>' );
				...
				it( 'should not convert in wrong context', () => {
					model.schema.register( 'div', { inheritAllFrom: '$block' } );
					model.schema.addChildCheck( ( ctx, childDef ) => {
						if ( ctx.endsWith( 'div' ) && childDef.name === 'imageInline' ) {
							return false;
						}
					} );
				...
				it( 'should not convert "a" element if is already consumed', () => {
					...
					expect( editor.getData() ).to.equal(
						'<p><img src="/assets/sample.png" alt="alt text"></p>'
					);
				} );

				it( 'should not convert if a link misses "href" attribute', () => {
					editor.setData(
						'<p><a href=""><span class="image-inlne"><img src="/assets/sample.png" alt="alt text" /></span></a></p>'
					);
				...
				it( 'should convert a link without an image to a paragraph with the link', () => {
					editor.setData(
						'<a href="http://ckeditor.com"><span class="image-inline">Foo</span></a>'
					);
			...
			describe( 'a > img', () => {
				it( 'should convert a link in an inline image', () => {
					editor.setData(
				...
				it( 'should convert an image with a link and without alt attribute', () => {
					...
					expect( getModelData( model, { withoutSelection: true } ) ).to.equal(
						'<paragraph>' +
							'<imageInline linkHref="http://ckeditor.com" src="/assets/sample.png"></imageInline>' +
						'</paragraph>'
					);
				} );
				...
				it( 'should not convert in wrong context', () => {
					model.schema.register( 'div', { inheritAllFrom: '$block' } );
					model.schema.addChildCheck( ( ctx, childDef ) => {
						if ( ctx.last.name === 'div' && childDef.name === 'imageInline' ) {
							return false;
						}
					} );
	...
	describe( 'conversion in editing pipeline (block images)', () => {
		describe( 'model to view', () => {
	...
	describe( 'conversion in editing pipeline (inline images)', () => {
		describe( 'model to view', () => {
			it( 'should convert the image element', () => {
				setModelData( model,
					'<paragraph>' +
						'<imageInline linkHref="http://ckeditor.com" src="/assets/sample.png" alt="alt text"></imageInline>' +
					'</paragraph>'
				);

				expect( getViewData( view, { withoutSelection: true } ) ).to.equal(
					'<p><a href="http://ckeditor.com">' +
						'<span class="ck-widget image-inline" contenteditable="false">' +
							'<img alt="alt text" src="/assets/sample.png"></img>' +
						'</span>' +
						// Content of the UIElement is skipped here.
			...
			it( 'should convert attribute change', () => {
				...
				expect( getViewData( view, { withoutSelection: true } ) ).to.equal(
					'<p><a href="https://ckeditor.com/why-ckeditor/">' +
						'<span class="ck-widget image-inline" contenteditable="false">' +
							'<img alt="alt text" src="/assets/sample.png"></img>' +
						'</span>' +
						// Content of the UIElement is skipped here.
	...
	describe( 'link attributes decorator (block images)', () => {
			...
			describe( 'mode: automatic', () => {
				...
				testLinks.forEach( link => {
					it( `Link: ${ link.url } should get attributes: ${ JSON.stringify( link.attributes ) }`, () => {
						...
						editor.setData( `<figure><a href="${ link.url }"><img src="/assets/sample.png"></a></figure>` );
		...
		describe( 'upcast converter', () => {
			...
			it( 'should register manual decorators', async () => {
			...
			it( 'should register manual decorators when only block plugin is enabled', async () => {
				const newEditor = await VirtualTestEditor.create( {
	...
	describe( 'link attributes decorator (inline images)', () => {
		describe( 'default behavior', () => {
			const testLinks = [
				{
					external: true,
			...
			describe( 'for link.addTargetToExternalLinks=false', () => {
				...
				testLinks.forEach( link => {
					it( `link: ${ link.url } should not get 'target' and 'rel' attributes`, () => {
						// Upcast check.
						editor.setData(
							`<p><a href="${ link.url }" target="_blank" rel="noopener noreferrer">` +
									'<img src="/assets/sample.png">' +
								'</span>' +
							'</a></p>'
						);
						...
						expect( getModelData( model, { withoutSelection: true } ) )
							.to.equal(
								`<paragraph><imageInline linkHref="${ link.url }" src="/assets/sample.png"></imageInline></paragraph>`
							);
		...
		describe( 'custom config', () => {
			describe( 'mode: automatic', () => {
				...
				testLinks.forEach( link => {
					it( `Link: ${ link.url } should get attributes: ${ JSON.stringify( link.attributes ) }`, () => {
						const ORDER = [ 'class', 'target', 'download', 'href' ];
						const attributes = Object.assign( {}, link.attributes, {
							href: link.url
						} );
						const attr = Object.entries( attributes ).sort( ( a, b ) => {
		...
		describe( 'upcast converter', () => {
			...
			it( 'should register manual decorators when only image inline plugin is enabled', async () => {
				const newEditor = await VirtualTestEditor.create( {
			...
			it( 'should upcast attributes', async () => {
				...
				expect( getModelData( model, { withoutSelection: true } ) ).to.equal(
					'<paragraph>' +
						'<imageInline linkHref="url" linkIsDownloadable="true" linkIsExternal="true" src="/assets/sample.png">' +
						'</imageInline>' +
					'</paragraph>'
				);
			...
			it( 'should not upcast partial and incorrect attributes', async () => {
				...
				expect( getModelData( model, { withoutSelection: true } ) ).to.equal(
					'<paragraph><imageInline linkHref="url" linkIsExternal="true" src="/assets/sample.png"></imageInline></paragraph>'
				);
			} );
			...
			it( 'allows overwriting conversion process using highest priority', () => {
				...
				expect( editor.getData() ).to.equal( '<p><a href="url"><img src="/assets/sample.png"></a></p>' );
			} );

			it( 'should upcast the decorators when linked image (a > span > img)', () => {
				// (#7975)
				editor.setData(
					'<p><a class="gallery" href="https://cksource.com" target="_blank" rel="noopener noreferrer" download="download">' +
						'<span class="image-inline">' +
							'<img src="sample.jpg" alt="bar">' +
						'</span>' +
					'</a></p>' +
		...
		describe( 'downcast converter', () => {
			...
			it( 'should downcast the decorators after applying a change', () => {
				setModelData( model,
					'<paragraph>[<imageInline alt="bar" src="sample.jpg"></imageInline>]</paragraph>' +
					'<paragraph>' +
					'<$text>https://cksource.com</$text>' +
					'</paragraph>'
				);
				...
				expect( editor.getData() ).to.equal(
					'<p><a class="gallery" href="https://cksource.com" download="download" target="_blank" rel="noopener noreferrer">' +
						'<img src="sample.jpg" alt="bar">' +
					'</a></p>' +
					'<p>' +
						'<a class="gallery" href="https://cksource.com" download="download" target="_blank" rel="noopener noreferrer">' +
			...
			it( 'should downcast without error if the image already has no link', () => {
				...
				expect( editor.getData() ).to.equal(
					'<p><img src="sample.jpg" alt="bar"></p>'
				);
			} );
			...
			describe( 'order of model updates', () => {
				it( 'should not affect converters - base link attributes first', () => {
					setModelData( model,
						'<paragraph>[<imageInline src="https://cksource.com"></imageInline>]</paragraph>'
					);
					...
					expect( getModelData( model, { withoutSelection: true } ) ).to.equal(
						'<paragraph>' +
							'<imageInline linkHref="url" linkIsDownloadable="true" src="https://cksource.com"></imageInline>' +
						'</paragraph>'
					);
				} );
