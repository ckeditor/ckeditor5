...
import {jscolor} from '@eastdesire/jscolor/jscolor.js';
...
export default class ColorpickerInputView extends View {
	 ...
	 */
	constructor(locale, { columns, removeButtonLabel, documentColorsLabel, documentColorsCount }) {
		super(locale);
		...
		const bind = inputField.bindTemplate;

		inputField.setTemplate({
			tag: 'input',
			attributes: {
				/* type: 'color', */
				'data-jscolor': "{}",
		...
		const removeColorBtn = this._removeColorButton();

		this.items.add(removeColorBtn);

		removeColorBtn.delegate('execute').to(this, 'input');
	...
	/**
	 * Adds a given color to the document colors list.
	 *
	 * @private
	 * @param {String} color A string that stores the value of the recently applied color.
	 */
	_addColorToDocumentColors(color) {
		const predefinedColor = this.documentColors
