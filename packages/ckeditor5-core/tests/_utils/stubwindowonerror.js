 ...
 * the `window` listeners under test) and assert the errors that reached `window.onerror`.
 *
 * `CKEditorError` errors — the ones these tests throw on purpose — are swallowed silently. Any other
 * error caught by the stub, including one thrown while the test cleans up (for example during
 * `editor.destroy()`), is re-thrown when the test finishes and fails the test, so the stub does not
 * hide unexpected failures. The test runner alone would only log such errors without failing anything,
 * as its error tracker mutes itself while other `error` listeners are registered. Tests that
