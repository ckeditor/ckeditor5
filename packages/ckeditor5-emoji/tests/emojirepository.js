...
describe( 'EmojiRepository', () => {
	...
	let isEmojiSupportedStub, consoleStub, fetchStub, clock;
	...
	describe( 'init()', () => {
		...
		it( 'should filter out unsupported emojis from the fetched emoji database', async () => {
			...
			const database = EmojiRepository._database[ 16 ];
			const hasMicroscopeEmoji = database.some( item => item.annotation === 'microscope' );

			expect( hasMicroscopeEmoji ).to.equal( false );
