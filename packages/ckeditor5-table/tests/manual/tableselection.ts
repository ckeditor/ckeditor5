...
function createEditor( target: string, inspectorName: string ) {
	ClassicEditor
		.create( {
			attachTo: document.querySelector( target ) as HTMLElement,
...
function printModelContents( editor: ClassicEditor ) {
...
function formatTable( tableString: string ) {
	return tableString
		.replace( /<table>/g, '\n<table' )
		.replace( /<table /g, '\n<table ' )
		.replace( /<tableRow>/g, '\n<tableRow>\n    ' )
