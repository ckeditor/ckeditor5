...
ClassicEditor
	...
	.then( editor => {
		...
		const modelData = document.getElementById( 'model-data' ) as HTMLTextAreaElement;
		...
		document.getElementById( 'set-model-data' )!.addEventListener( 'click', () => {
			...
			if ( inputModelData ) {
				const element = _setModelData._parse( modelTable( inputModelData ), editor.model.schema ) as ModelNode;
		...
		document.getElementById( 'renumber-cells' )!.addEventListener( 'click', () => {
			...
			const useLetters = ( document.getElementById( 'use-letters' ) as HTMLInputElement ).checked;
		...
		function findTable( editor: ClassicEditor, useAnyTable = false ) {
		...
		function getAllTables( editor: ClassicEditor ) {
			const range = editor.model.createRangeIn( editor.model.document.getRoot()! );
			const tables = [];
		...
		function parseModelData( string: string ) {
			if ( !string.trim() ) {
				return null;
			}
		...
		function createCellText( row: number, column: number, useLetters: boolean ) {
...
function diffString( left: any, right: any, options: { theme?: Record<string, ( string: string ) => string> } = {} ) {
