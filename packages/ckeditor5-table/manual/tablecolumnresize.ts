/**
 * @license Copyright (c) 2003-2026, CKSource Holding sp. z o.o. All rights reserved.
 * For licensing, see LICENSE.md or https://ckeditor.com/legal/ckeditor-licensing-options
 */

import { ClassicEditor } from '@ckeditor/ckeditor5-editor-classic';
import { ArticlePluginSet } from '@ckeditor/ckeditor5-core/tests/_utils/articlepluginset.js';
import { Table } from '../src/table.js';
import { TableToolbar } from '../src/tabletoolbar.js';
import { TableSelection } from '../src/tableselection.js';
import { TableClipboard } from '../src/tableclipboard.js';
import { TableProperties } from '../src/tableproperties.js';
import { TableCellProperties } from '../src/tablecellproperties.js';
import { TableCaption } from '../src/tablecaption.js';
import { TableColumnResize } from '../src/tablecolumnresize.js';

declare global {
	interface Window {
		editor: any;
		editorRTL: any;
	}
}

const editorConfig = {
	image: { toolbar: [ 'toggleImageCaption', 'imageTextAlternative' ] },
	plugins: [
		ArticlePluginSet,
		Table,
		TableToolbar,
		TableSelection,
		TableClipboard,
		TableProperties,
		TableCellProperties,
		TableCaption,
		TableColumnResize
	],
	toolbar: [
		'heading', '|',
		'insertTable', '|',
		'bold', 'italic', 'link', '|',
		'bulletedList', 'numberedList', 'blockQuote', '|',
		'undo', 'redo'
	],
	table: {
		contentToolbar: [
			'tableColumn',
			'tableRow',
			'mergeTableCells',
			'tableProperties',
			'tableCellProperties',
			'toggleTableCaption'
		]
	}
};

ClassicEditor
	.create( {
		...editorConfig,
		attachTo: document.querySelector( '#editor' ) as HTMLElement
	} )
	.then( editor => {
		window.editor = editor;
	} )
	.catch( err => {
		console.error( err.stack );
	} );

ClassicEditor
	.create( Object.assign( {}, editorConfig, {
		attachTo: document.querySelector( '#editor-rtl' ) as HTMLElement,
		language: 'ar'
	} ) )
	.then( editor => {
		window.editorRTL = editor;
	} )
	.catch( err => {
		console.error( err.stack );
	} );

document.querySelector( 'button#read-only' )!.addEventListener( 'click', () => {
	if ( window.editor.isReadOnly ) {
		window.editor.disableReadOnlyMode( 'test' );
	} else {
		window.editor.enableReadOnlyMode( 'test' );
	}

	if ( window.editorRTL.isReadOnly ) {
		window.editorRTL.disableReadOnlyMode( 'test' );
	} else {
		window.editorRTL.enableReadOnlyMode( 'test' );
	}
} );
