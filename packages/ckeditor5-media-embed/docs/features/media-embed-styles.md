...
<info-box warning>
	Only one style should be marked as the default. If multiple are marked, the first one in the resolved options wins. If none is marked, the plugin falls back to the built-in `alignCenter` as the effective default. Even when `alignCenter` is not in the configured options, executing the command with that value still clears the attribute.
</info-box>
