# Media embed styles (default config) without the resize feature

Editor configured with `MediaEmbed` + `MediaEmbedStyle` (no `MediaEmbedResize`) and the default
five-built-in style set. The page stylesheet caps the YouTube figure at `max-width: 50%`, simulating
a scenario where the figure's width comes from outside the resize feature (custom integrator CSS,
GeneralHtmlSupport preserving inline styles, custom upcast, etc.). The Spotify embed is unaffected
by that rule and demos a different width source: its built-in 300px default.
...
1. The data `<figure>` should be `class="media"` only, with no alignment class.
...
1. The data `<figure>` should include `media-style-block-align-left` (no `media_resized` class, because the resize plugin is not loaded),
