...
class MultiRootEditorIntegration {
	...
	public init() {
		this.editor.on( 'addRoot', ( evt: any, root: any ) => {
			this.addRoot( root );
		} );

		this.editor.on( 'detachRoot', ( evt: any, root: any ) => {
			this.detachRoot( root );
		} );
	}

	public addRoot( root: any ) {
		const domElement = this.editor.createEditable( root );

		this.attachEditable( domElement );
	}

	public attachEditable( domElement: HTMLElement ) {
...
function createWatchdog( stateElement: HTMLElement ) {
	const watchdog = new EditorWatchdog( MultiRootEditor );

	watchdog.setCreator( ( config: any ) => {
