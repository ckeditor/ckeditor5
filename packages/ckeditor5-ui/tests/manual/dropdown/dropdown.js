...
function testToolbar() {
	...
	const buttons = Object.keys( icons ).map( icon => new ViewModel( { label: icon, isEnabled: true, isOn: false, icon: icons[ icon ] } ) );

	const buttonViews = buttons
		.map( buttonViewModel => {
			const buttonView = new ButtonView( locale );

			buttonView.bind( 'isEnabled', 'isOn', 'icon', 'label' ).to( buttonViewModel );

			buttonView.on( 'execute', () => console.log( `Execute: ${ buttonViewModel.label }` ) );
