...
function toolbar( children: Array<View> = [] ) {
...
function toolbarDropdown( {
	...
	dropdown.buttonView.set( { label, isEnabled, isVertical, isOn, withText } as any );
...
function splitButtonDropdown( {
	...
	dropdown.buttonView.set( { icon, label, isEnabled, isVertical, isOn, withText } as any );
...
function input( {
	...
	const labeledField = new LabeledFieldView( {} as Locale, createLabeledInputText );

	labeledField.set( { isEnabled, label, value } as any );

	return labeledField;
}

function setManualTestDirection( direction: string ) {
...
document.querySelector( '#direcion-selector' )!.addEventListener( 'change', evt => {
	setManualTestDirection( ( evt.target as HTMLSelectElement ).value );
} );
