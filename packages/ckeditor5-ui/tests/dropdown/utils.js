	...
	describe( 'addListToDropdown()', () => {
		...
		describe( 'view#listView', () => {
			...
			it( 'should be created before chained observables are updated', () => {
				...
				definitions.add( {
					type: 'button',
					model: new ViewModel( { label: 'a' } )
				} );

				definitions.add( {
					type: 'button',
					model: new ViewModel( { label: 'b' } )
				} );
			...
			describe( 'with ListItemButtonView', () => {
				it( 'is populated using item definitions', () => {
					definitions.add( {
						type: 'button',
						model: new ViewModel( { label: 'a', labelStyle: 'b' } )
					} );

					definitions.add( {
						type: 'button',
						model: new ViewModel( { label: 'c', labelStyle: 'd' } )
					} );
				...
				it( 'should set `isToggleable=true` only if role `menuitemcheckbox` or `menuitemradio` is set', () => {
					definitions.addMany( [
						{
							type: 'button',
							model: new ViewModel( { label: 'a', role: 'menuitemcheckbox' } )
						},
						{
							type: 'button',
							model: new ViewModel( { label: 'b', role: 'menuitemradio' } )
						},
						{
							type: 'button',
							model: new ViewModel( { label: 'c', role: 'menuitem' } )
						}
					] );
				...
				it( 'should reserve checkbox holder space on non-toggleable items', () => {
					definitions.addMany( [
						{
							type: 'button',
							model: new ViewModel( { label: 'a', role: 'menuitem' } )
						},
						{
							type: 'button',
							model: new ViewModel( { label: 'b', role: 'menuitem' } )
						},
						{
							type: 'button',
							model: new ViewModel( { label: 'c', role: 'menuitemradio' } )
						}
					] );
				...
				it( 'binds all button properties', () => {
					const def = {
						type: 'button',
						model: new ViewModel( { label: 'a', labelStyle: 'b', foo: 'bar', baz: 'qux' } )
					};
			...
			describe( 'with ListGroupView', () => {
				...
				beforeEach( () => {
					definitionsWithGroups = [
						{
							type: 'button',
							model: new ViewModel( { label: 'a', labelStyle: 'x' } )
						},
						{
							type: 'group',
							label: 'b',
							items: new Collection( [
								{
									type: 'button',
									model: new ViewModel( { label: 'b.a', labelStyle: 'y' } )
								},
								{
									type: 'button',
									model: new ViewModel( { label: 'b.b', labelStyle: 'z' } )
								}
							] )
						}
					];
		...
		describe( 'focus management on dropdown open', () => {
			...
			it( 'focuses active item upon dropdown opening', () => {
				definitions.addMany( [
					{
						type: 'button',
						model: new ViewModel( { label: 'a', isOn: true } )
					},
					{
						type: 'button',
						model: new ViewModel( { label: 'b' } )
			...
			it( 'focuses nth active item upon dropdown opening', () => {
				definitions.addMany( [
					...
					{
						type: 'button',
						model: new ViewModel( { label: 'b', isOn: true } )
					}
				] );
			...
			it( 'focuses the first item if multiple items are active', () => {
				definitions.addMany( [
					...
					{
						type: 'button',
						model: new ViewModel( { label: 'c', isOn: true } )
					}
				] );
