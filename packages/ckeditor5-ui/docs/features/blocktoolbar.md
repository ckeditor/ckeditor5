...
The block toolbar comes in handy when the main editor toolbar cannot be accessed. It complements the {@link installation/predefined-builds#balloon-editor Balloon editor} when it falls short, for example when some content must be inserted (like an image) but the selection is collapsed, leaving the user unable to access the toolbar. You can read more about it in the {@link installation/predefined-builds#balloon-block-editor balloon block editor overview}.
