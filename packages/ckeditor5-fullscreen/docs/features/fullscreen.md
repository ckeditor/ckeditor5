...
`enableCallback( container )` is fired when fullscreen mode is enabled. Passed parameter is the final DOM element container generated by the default fullscreen behavior. It contains the whole fullscreen mode DOM structure, already set up (the editor UI is already injected into the container). You are free to perform any changes, e.g. relocate sidebars or append additional UI elements.

`disableCallback( container )` is fired when fullscreen mode is disabled. Passed parameter is the container with the fullscreen mode DOM structure. The callback should be used to take care of any of your custom components, e.g. if some elements were moved to the fullscreen DOM structure, restore their original locations in the DOM.
