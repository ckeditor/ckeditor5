...
If you want to display some additional elements in the fullscreen mode, modify the `config.fullscreen.onEnterCallback` and `config.fullscreen.onLeaveCallback` properties.

`onEnterCallback( container )` is fired when you enter fullscreen mode. Passed parameter is the final DOM element container generated by the default fullscreen behavior. It contains the whole fullscreen mode DOM structure, already set up (the editor UI is already injected into the container). You are free to perform any changes, e.g. relocate sidebars or append additional UI elements.
