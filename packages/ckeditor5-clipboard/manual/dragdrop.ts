...
class HCardEditing extends Plugin {
	...
	public _defineConverters() {
		...
		function createCardView( modelItem: any, viewWriter: any ) {
	...
	public _defineClipboardInputOutput() {
		...
		this.listenTo( document as any, 'clipboardOutput', ( evt, data: any ) => {
...
function getCardDataFromViewElement( viewElement: any ) {
	const children: Array<any> = Array.from( viewElement.getChildren() );
...
function getText( viewElement: any ) {
	return Array.from( viewElement.getChildren() )
		.map( ( node: any ) => node.is( '$text' ) ? node.data : '' )
		.join( '' );
}
...
contactsContainer!.addEventListener( 'dragstart', ( event: any ) => {
...
contacts.forEach( ( contact, id ) => {
	...
	li.innerHTML =
		`<div class="contact h-card" data-contact="${ id }" draggable="true">` +
			`<img src="/images/${ contact.avatar }.png" alt="avatar" class="u-photo" draggable="false" />` +
			contact.name +
		'</div>';
...
dropArea!.addEventListener( 'drop', ( event: any ) => {
	...
	( dropArea as HTMLElement ).innerText =
		'-- text/plain --\n' + event.dataTransfer.getData( 'text/plain' ) + '\n\n' +
		'-- text/html --\n' + event.dataTransfer.getData( 'text/html' ) + '\n\n' +
		'-- contact --\n' + ( contact ? JSON.stringify( JSON.parse( contact ), null, 2 ) : '' ) + '\n';
	dropArea!.classList.remove( 'dragover' );
