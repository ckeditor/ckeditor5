...
ClassicEditor
	.create( document.querySelector( '#editor-with-image-editor-overwrite-false' ), {
		plugins: [
	...
	.then( editor => {
		window.editorWithoutOverwrite = editor;

		CKEditorInspector.attach( { editorWithoutOverwrite: editor } );
	} )
	.catch( err => {
...
ClassicEditor
	.create( document.querySelector( '#editor-with-image-editor-overwrite-true' ), {
		plugins: [
			ArticlePluginSet, PictureEditing, ImageUpload, LinkImageEditing,
			ImageInsert, CloudServices, CKBox, LinkImage, CKBoxImageEdit
		],
	...
	.then( editor => {
		window.editorWithOverwrite = editor;

		CKEditorInspector.attach( { editorWithOverwrite: editor } );
	} )
	.catch( err => {
		console.error( err.stack );
	} );
