...
describe( 'CKBoxImageEditCommand', () => {
	...
	describe( 'events', () => {
		...
		describe( 'saving edited asset', () => {
			...
			it( 'should abort the request if cannot determine a response due to network error', () => {
				const spy = sinon.spy( command.controller.signal, 'throwIfAborted' );
			...
			it( 'should throw an error on abort', () => {
				...
				const promise = command._getAssetStatusFromServer( dataMock )
					.then( () => {
						throw new Error( 'Promise should throw.' );
					} )
					.catch( () => {
						expect( command.controller.signal.aborted ).to.equal( true );
					} );
