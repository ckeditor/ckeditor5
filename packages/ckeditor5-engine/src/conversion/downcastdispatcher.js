 ...
 *
 * Additionally, downcast dispatcher fires events for the {@link module:engine/model/markercollection~Marker marker} changes:
 *
 ...
 *
 * You can read more about conversion in the dedicated {@glink framework/guides/deep-dive/conversion/intro Conversion} guides
 * in the CKEditor 5 Framework documentation.
 *
...
export default class DowncastDispatcher {
	...
	/**
	 * Takes a {@link module:engine/model/differ~Differ model differ} object with buffered changes and fires the conversion basing on it.
	 *
	 ...
	 *
	 * For each node in the range, an {@link #event:insert `insert` event} is fired. For each attribute on each node,
	 * an {@link #event:attribute `attribute` event} is fired.
	 *
	 * @fires insert
	 * @fires attribute
	...
	/**
	 * Returns the differ changes together with the added "reconvert" type changes for {@link #reconvertElement}.
	 * These are defined by the `triggerBy()` configuration for the
	 * {@link module:engine/conversion/downcasthelpers~DowncastHelpers#elementToElement `elementToElement()`} conversion helper.
	 *
	 ...
	 * `insert:name`. `name` is either `'$text'`, when a {@link module:engine/model/text~Text text node} has been inserted,
	 * or {@link module:engine/model/element~Element#name name} of inserted element.
	 *
	 * This way, the listeners can either listen to a general `insert` event or a specific event (for example `insert:paragraph`).
	 *
	 * @event insert
	 ...
	 * `remove:name`. `name` is either `'$text'`, when a {@link module:engine/model/text~Text text node} has been removed,
	 ...
	 * @event attribute
	 * @param {Object} data Additional information about the change.
	 * @param {module:engine/model/item~Item|module:engine/model/documentselection~DocumentSelection} data.item The changed item
	 * or converted selection.
	 * @param {module:engine/model/range~Range} data.range Range spanning over the changed item or selection range.
