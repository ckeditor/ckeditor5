...
const byClassName = ( className: string ) => ( element: any ) => element.hasClass( className );

const getRandom = () => parseInt( Math.random() * 1000 as any );

function mapMeta( editor: Editor ) {
	return ( metaElement: any ) => {
...
function getChildren( editor: Editor, viewElement: any ): Array<any> {
	return Array.from( editor.editing.view.createRangeIn( viewElement ) )
		.filter( ( { type } ) => type === 'elementStart' )
		.map( ( { item } ) => item );
}

function getBoxUpcastConverter( editor: Editor ) {
	return ( dispatcher: UpcastDispatcher ) => dispatcher.on( 'element:div', ( event, data, conversionApi ) => {
...
function getBoxDowncastCreator( multiSlot: any ) {
	return ( modelElement: any, conversionApi: any ) => {
		...
		for ( const [ meta, metaValue ] of Object.entries<any>( modelElement.getAttribute( 'meta' ) ) ) {
			if ( meta === 'header' ) {
		...
		if ( !multiSlot ) {
			...
			writer.insert( writer.createPositionAt( contentWrap, 0 ), writer.createSlot( ( element: any ) => element.index < 2 ) );
			...
			writer.insert( writer.createPositionAt( contentWrap2, 0 ), writer.createSlot( ( element: any ) => element.index >= 2 ) );

			const footer = writer.createRawElement( 'div', { class: 'box-footer' }, ( domElement: any ) => {
...
function addButton( editor: Editor, uiName: string, label: string, callback: any ) {
	editor.ui.componentFactory.add( uiName, locale => {
		...
		view.listenTo( view, 'execute', () => {
			const parent: any = editor.model.document.selection.getFirstPosition()!.parent;
...
function addBoxMetaButton( editor: Editor, uiName: string, label: string, updateWith: any ) {
	addButton( editor, uiName, label, ( writer: any, box: any ) => {
...
function Box( editor: Editor ) {
	...
	addButton( editor, 'addBoxField', '+', ( writer: any, box: any, boxField: any ) => {
	...
	addButton( editor, 'removeBoxField', '-', ( writer: any, box: any, boxField: any ) => {
		writer.remove( boxField );
	} );
}

function AddRenderCount( editor: Editor ) {
	let insertCount = 0;
...
async function createEditor( multiSlot: boolean ) {
...
for ( const option of document.querySelectorAll<HTMLInputElement>( 'input[name=box-mode]' ) ) {
	option.addEventListener( 'change', async event => {
		if ( window.editor ) {
			await window.editor.destroy();
		}

		await createEditor( ( event.target as HTMLInputElement ).value == 'multi' );
	} );
