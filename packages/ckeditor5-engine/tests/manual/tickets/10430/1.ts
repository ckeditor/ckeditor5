...
import type { ModelElement } from '../../../../src/index.js';
declare global {
	interface Window {
		editor: any;
		updateSelectionRenderingInfo: any;
	}
}
...
ClassicEditor
	...
	.then( editor => {
		...
		editor.model.change( writer => {
			const range = writer.createRangeIn( editor.model.document.getRoot()!.getChild( 0 ) as ModelElement );
			const name = 'highlight:1234';

			writer.addMarker( name, { range, usingOperation: false } );
		} );

		window.updateSelectionRenderingInfo = function( isBlocked: boolean ) {
