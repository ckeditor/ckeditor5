...
describe( 'ViewDomConverter', () => {
	...
	describe( 'shouldRenderAttribute()', () => {
		...
		it( 'should reject data: URIs that the browser can load as a document', () => {
			...
			expect( converter.shouldRenderAttribute( 'src', 'data:text/xml,foo', 'iframe' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', 'data:application/xml,foo', 'iframe' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', 'data:application/xhtml+xml,foo', 'iframe' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', 'data:text/xsl,foo', 'iframe' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', 'data:application/rss+xml,foo', 'iframe' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', 'data:application/atom+xml,foo', 'iframe' ) ).to.be.false;

			// MIME types blocked before the fix as well.
			expect( converter.shouldRenderAttribute( 'src', 'data:text/html,foo', 'iframe' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', 'data:text/xhtml,foo', 'iframe' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', 'data:image/svg+xml,foo', 'iframe' ) ).to.be.false;

			// The same payload in the remaining document-loading contexts.
			expect( converter.shouldRenderAttribute( 'data', 'data:image/svg+xml,foo', 'object' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', 'data:image/svg+xml,foo', 'embed' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', 'data:text/xml,foo', 'frame' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'href', 'data:text/html,foo', 'a' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', 'data:text/javascript,alert(1)', 'script' ) ).to.be.false;

			// Elements unknown to the converter (for instance enabled via General HTML Support).
			expect( converter.shouldRenderAttribute( 'src', 'data:text/xml,foo', 'ck-custom-widget' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', 'data:text/xml,foo' ) ).to.be.false;
		} );

		it( 'should reject data: URIs regardless of the letter case and whitespace', () => {
			expect( converter.shouldRenderAttribute( 'src', 'DATA:TEXT/XML,foo', 'iframe' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', '   data:text/xml,foo', 'iframe' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', 'da ta:te\txt/xml,foo', 'iframe' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', 'data:text/xml;base64,PHN2Zy8+', 'iframe' ) ).to.be.false;
		} );

		const safeDataUriMimeTypes = [
			'image/png',
			'image/jpeg',
			'image/jpg',
			'image/gif',
		...
		for ( const mimeType of safeDataUriMimeTypes ) {
			it( `should allow a data: URI of the ${ mimeType } type`, () => {
				const url = `data:${ mimeType };base64,AAA`;

				expect( converter.shouldRenderAttribute( 'src', url, 'iframe' ) ).to.be.true;
				expect( converter.shouldRenderAttribute( 'src', url, 'img' ) ).to.be.true;
				expect( converter.shouldRenderAttribute( 'srcset', url, 'img' ) ).to.be.true;
				expect( converter.shouldRenderAttribute( 'srcset', url, 'source' ) ).to.be.true;
				expect( converter.shouldRenderAttribute( 'src', url, 'video' ) ).to.be.true;
				expect( converter.shouldRenderAttribute( 'src', url, 'audio' ) ).to.be.true;
			} );
		}

		it( 'should allow an allow listed data: URI regardless of its letter case and parameters', () => {
			expect( converter.shouldRenderAttribute( 'src', 'DATA:IMAGE/PNG;base64,AAA', 'iframe' ) ).to.be.true;
			expect( converter.shouldRenderAttribute( 'src', 'data:audio/ogg;codecs=opus;base64,AAA', 'audio' ) ).to.be.true;
			expect( converter.shouldRenderAttribute( 'src', 'data:image/png,foo', 'iframe' ) ).to.be.true;
			expect( converter.shouldRenderAttribute( 'src', 'data:image/png', 'iframe' ) ).to.be.true;
		} );

		it( 'should not affect values that are not data: URIs', () => {
			expect( converter.shouldRenderAttribute( 'src', 'https://example.com/foo.png', 'iframe' ) ).to.be.true;
			expect( converter.shouldRenderAttribute( 'src', '/foo.png', 'iframe' ) ).to.be.true;
			expect( converter.shouldRenderAttribute( 'anything', 'foobar data:text/xml,foo' ) ).to.be.true;
		} );
		...
		it( 'should allow any data: URI in the img and source exceptions', () => {
			...
			expect( converter.shouldRenderAttribute( 'src', 'data:image/svg+xml,foo', 'img' ) ).to.be.true;
			expect( converter.shouldRenderAttribute( 'srcset', 'data:image/svg+xml,foo', 'img' ) ).to.be.true;
			expect( converter.shouldRenderAttribute( 'srcset', 'data:image/svg+xml,foo', 'source' ) ).to.be.true;
			expect( converter.shouldRenderAttribute( 'src', 'data:text/xml,foo', 'img' ) ).to.be.true;
		} );
		...
		it( 'should reject SVG and other non-listed data: URIs outside of the img and source exceptions', () => {
			expect( converter.shouldRenderAttribute( 'src', 'data:image/svg+xml,foo', 'source' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'poster', 'data:image/svg+xml,foo', 'video' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', 'data:text/vtt,WEBVTT', 'track' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'data', 'data:application/pdf;base64,AAA', 'object' ) ).to.be.false;
		} );
		...
		it( 'should match the allowed subtypes exactly, not as a prefix', () => {
			...
			expect( converter.shouldRenderAttribute( 'src', 'data:image/png+xml,foo', 'iframe' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', 'data:video/mp4+xml,foo', 'iframe' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', 'data:audio/mpeg+xml,foo', 'iframe' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', 'data:image/pngfoo,foo', 'iframe' ) ).to.be.false;

			// The allowed subtypes followed by a parameter, by the data or by nothing at all.
			expect( converter.shouldRenderAttribute( 'src', 'data:image/png;base64,AAA', 'iframe' ) ).to.be.true;
			expect( converter.shouldRenderAttribute( 'src', 'data:image/png,foo', 'iframe' ) ).to.be.true;
			expect( converter.shouldRenderAttribute( 'src', 'data:video/mp4', 'iframe' ) ).to.be.true;
		} );
		...
		it( 'should reject URIs with a scheme hidden behind control characters', () => {
			...
			expect( converter.shouldRenderAttribute( 'src', '\u001fdata:text/xml,foo', 'iframe' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', '\u0001\u0002data:image/svg+xml,foo', 'iframe' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'href', '\u0001javascript:alert(1)', 'a' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'href', '\u001b JAVASCRIPT:alert(1)', 'a' ) ).to.be.false;
		...
		it( 'should reject javascript: URIs split with whitespace', () => {
			expect( converter.shouldRenderAttribute( 'src', 'jav\tascript:alert(1)', 'iframe' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'src', 'jav\nascript:alert(1)', 'iframe' ) ).to.be.false;
			expect( converter.shouldRenderAttribute( 'href', 'JAVASCRIPT:alert(1)', 'a' ) ).to.be.false;
		} );
		...
		it( 'should allow all the newly blocked data: URIs in the data pipeline', () => {
			...
			expect( converter.shouldRenderAttribute( 'src', 'data:text/xml,foo', 'iframe' ) ).to.be.true;
			expect( converter.shouldRenderAttribute( 'src', 'data:application/xhtml+xml,foo', 'iframe' ) ).to.be.true;
			expect( converter.shouldRenderAttribute( 'data', 'data:image/svg+xml,foo', 'object' ) ).to.be.true;
		} );
	} );
	...
	describe( 'setContentOf()', () => {
		...
		describe( 'editing pipeline', () => {
			...
			it( 'should rename the src attribute of an iframe loading an unsafe data: URI', () => {
				...
				expect( element.innerHTML ).to.equal( `<iframe data-ck-unsafe-attribute-src="${ payload }"></iframe>` );
			...
			it( 'should keep data: URIs of binary images in the raw content', () => {
				...
				expect( element.innerHTML ).to.equal( `<img src="${ payload }"><iframe src="${ payload }"></iframe>` );
			...
			it( 'should rename an attribute whose scheme is hidden behind a control character', () => {
				...
				expect( iframe.getAttribute( 'data-ck-unsafe-attribute-src' ) ).to.equal( '\u0001data:text/html,foo' );
	...
	describe( 'setDomElementAttribute()', () => {
		...
		it( 'should transform src attribute to unsafe for XML-based data: URIs on iframe element', () => {
			...
			expect( domElement.outerHTML ).to.equal( '<iframe data-ck-unsafe-attribute-src="data:text/xml,foo"></iframe>' );

			converter.setDomElementAttribute( domElement, 'src', 'data:application/xhtml+xml,foo' );
			expect( domElement.outerHTML ).to.equal(
				'<iframe data-ck-unsafe-attribute-src="data:application/xhtml+xml,foo"></iframe>'
			);
		} );
		...
		it( 'should set src attribute for binary image data: URIs on iframe element', () => {
			...
			expect( domElement.outerHTML ).to.equal( '<iframe src="data:image/png;base64,AAA"></iframe>' );
		} );
	} );
