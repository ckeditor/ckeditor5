...
import { DomConverter, StylesProcessor, ViewDocument, ViewUIElement, DowncastWriter } from '@ckeditor/ckeditor5-engine';
...
describe( 'BookmarkEditing', () => {
	let editor, element, model, view, viewDocument, converter, writer;
	...
	describe( 'editingDowncast', () => {
		...
		it( 'TODO: test getFillerOffset', () => {
			...
			// here filler is showing
			const viewElement = writer.createContainerElement( 'a', {} );

			const domElement1 = converter.viewToDom( viewElement );

			expect( domElement1.outerHTML ).to.equal( '<a></a>' );
		} );
	} );
} );
