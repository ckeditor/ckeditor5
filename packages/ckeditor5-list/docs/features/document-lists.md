...
Document lists offer formatting tools, just like regular lists. The list style feature introduces some more styles for the list item markers. When enabled, it adds 3 styles for unordered lists and 6 styles for ordered lists to choose from. The user can set or change the list style via the dropdown. It opens when you click the arrow next to the appropriate list button in the toolbar.
...
<info-box info>
	...
	The document lists feature is not enabled in any predefined builds, so you need to install it by hand.
</info-box>
...
ClassicEditor
	.create( document.querySelector( '#editor' ), {
		plugins: [ DocumentList, /* More plugins. */ ],
		toolbar: [ 'bulletedList', 'numberedList', /* More toolbar buttons. */ ]
	} )
	.then( /* ... */ )
	.catch( /* ... */ );
```
...
Then add the `DocumentListProperties` plugin to your plugin list and configure the toolbar. To enable selected sub-features of the list properties, add their configuration to your editor (set `true` for each feature you want to enable):
...
By default, the editor will merge two lists of the same type (ordered and unordered) that are next to each other. This is done so that lists that visually appear to be one continuous list actually are one list, even if the user has accidentally created several of them.
...
The simple list configuration option is a great solution for users who do not need to turn block elements into list items. When this setting is active, users can only insert text into list items. They will not be able to nest content blocks &ndash; like paragraphs or tables &ndash; inside a list item. This would be handy for small editing areas and for content creation solutions that need to work with less advanced documents. Turning off the default block support will make editing easier with limited capabilities and also affect some aspects like keyboard shortcuts.

You can turn the block list support off in the configuration:
