...
describe( 'ListEditing integrations: tab key', () => {
	...
	function runTest( { input, expected, domEventData, eventStopped, executedCommands = {}, changedBlocks = [], customSetModelData } ) {
		...
		if ( typeof eventStopped === 'object' ) {
			expect( domEventData.domEvent.stopPropagation.mock.calls.length > 0, 'stopPropagation() call' )
				.toEqual( eventStopped.stopPropagation );
			...
			expect( domEventData.domEvent.stopPropagation.mock.calls.length, 'stopPropagation() call' ).toEqual( eventStopped ? 1 : 0 );
