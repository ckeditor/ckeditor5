# List with skip levels (`enableSkipLevelLists: true`)

Editor with `list.enableSkipLevelLists` set to `true`. List items can be indented by more than one level relative to their parent.
