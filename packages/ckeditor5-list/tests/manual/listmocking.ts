...
ClassicEditor
	...
	.then( editor => {
		...
		( document.getElementById( 'data-input' ) as HTMLTextAreaElement ).value = model;
...
const getListModelWithNewLines = ( stringifiedModel: string ) => {
	return stringifiedModel.replace( /<\/(paragraph|heading\d)>/g, '</$1>\n' );
};

const setModelDataFromAscii = () => {
	const asciiList = ( document.getElementById( 'data-input' ) as HTMLTextAreaElement ).value;
...
const createAsciiListCodeSnippet = ( stringifiedAsciiList: string ) => {
...
const setAsciiListFromModel = () => {
	const editorModelString = ( document.getElementById( 'data-input' ) as HTMLTextAreaElement ).value;
...
const processInput = () => {
	const dataType = ( document.querySelector( 'input[name="input-type"]:checked' ) as HTMLInputElement ).value;
	...
	if ( ( document.getElementById( 'chbx-should-copy' ) as HTMLInputElement ).checked ) {
		copyOutput();
	}
};
...
const processEditorModel = () => {
	...
	if ( dataType === 'model' ) {
		...
		( document.getElementById( 'data-input' ) as HTMLTextAreaElement ).value = editorModelStringWithNewLines;
	}
	...
	if ( dataType === 'ascii' ) {
		...
		( document.getElementById( 'data-input' ) as HTMLTextAreaElement ).value =
			createAsciiListCodeSnippet( stringifyList( editorModel ) );
	}
...
const onPaste = () => {
	if ( ( document.getElementById( 'chbx-process-on-paste' ) as HTMLInputElement ).checked ) {
