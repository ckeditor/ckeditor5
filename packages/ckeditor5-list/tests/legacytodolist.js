...
describe( 'LegacyTodoList', () => {
	it( 'should be named', () => {
		expect( LegacyTodoList.pluginName ).toBe( 'LegacyTodoList' );
	} );

	it( 'should require LegacyTodoListEditing and TodoListUI', () => {
		expect( LegacyTodoList.requires ).toEqual( [ LegacyTodoListEditing, TodoListUI ] );
	} );

	it( 'should have `isOfficialPlugin` static flag set to `true`', () => {
		expect( LegacyTodoList.isOfficialPlugin ).toBe( true );
	} );

	it( 'should have `isPremiumPlugin` static flag set to `false`', () => {
		expect( LegacyTodoList.isPremiumPlugin ).toBe( false );
	} );
} );
