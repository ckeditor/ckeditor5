...
/**
 * Returns the upcast converter for skip-level list item wrappers. It detects intermediate `<li>` elements
 * with `list-style-type:none` (generated by the skip-level downcast or external sources) and unwraps them
 * so they don't produce empty list items in the model.
 *
 * The wrapper `<li>` is consumed without creating a model element, but its children (nested lists) are
 * converted normally. Because `getIndent()` counts all ancestor `<li>` elements (including the consumed wrapper),
 * the nested items receive the correct indent values that reflect the skip-level gap.
 *
 * @internal
 */
export function listItemSkipLevelUpcastConverter(): GetCallback<UpcastElementEvent> {
