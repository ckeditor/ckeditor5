 ...
 * * registering the {@link module:code-block/codeblockcaption/togglecodeblockcaptioncommand
 * ~ToggleCodeblockCaptionCommand `toggleCodeblockCaption`} command.
 *
...
export default class CodeblockCaptionEditing extends Plugin {
	 ...
	 * an codeblock element to allow restoring it in the future.
	 *
	 ...
	 *
	 * **Note**: The caption cannot be stored in the codeblock model element attribute because,
	 * for the instance, when the model state propagates to collaborators,
	 * the attribute would get lost (mainly because it does not convert to anything when the caption is hidden)
	 * and the states of collaborators' models would de-synchronize causing numerous issues.
